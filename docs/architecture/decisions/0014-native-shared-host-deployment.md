...
4. 端口固定为 staging app/edge 18110/18111，production app/edge 18112/18113；app 与 edge 都只绑定 loopback，不得绑定公网 80/443 或占用现有项目端口。
5. 项目目录、配置、发布制品、日志、测试资源和运行数据由 ubuntu 持有；服务用户 hearth 只通过专属服务组获得必要写权限。
6. staging 从 124 旧 Docker MySQL 只迁移 hearth logical database；Redis Session 不迁移。production 没有既有 Hearth 线上数据，首次部署初始化空 schema，不迁移 staging 用户、客户端或授权数据。
7. 任一 native 前置检查、数据迁移、健康检查、TLS/SNI 或只读验收失败，都不得影响其他项目。
8. Production Hearth 首次上线只初始化永久共享管理员 identity：复用 staging 既有管理员的 BCrypt hash，但创建 production issuer 下的新 identity；不复制其他 staging 用户、OAuth client 或授权状态，不创建临时账号。
9. Staging integration/E2E 使用 run-scoped 数据库与 Redis DB/namespace，并通过互斥 systemd test-slot 替代 staging app 执行；内存不足时 fail-closed，不停止其他项目。
...
| integration/E2E MySQL | hearth_integration_<RUN_ID> / h_it_<RUN_ID>；hearth_e2e_<RUN_ID> / h_e2e_<RUN_ID> | — | 每轮 staging 测试独立 schema/user |
| integration/E2E Redis | DB 12 / DB 13 | — | 唯一 run namespace；14/15 保留给 ByteDepth |
...
- 首次 production 部署需要以受控的一次性 hash 流程初始化管理员；只要生产 identity 表已非空，脚本不会覆盖或修复账号。
