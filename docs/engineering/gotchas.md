...
- staging integration 的 512 MiB transient cgroup 同时容纳 Maven 主 JVM 与 Failsafe fork；仅配置 PMD `skip` 仍会加载插件并增加 Maven 常驻内存。因本地/CI 门禁已运行 PMD，`staging-integration` profile 必须将 `pmd-check` execution 解绑（phase `none`），Failsafe fork heap 限在 128 MiB。
