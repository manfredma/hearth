...
readonly DEPLOY="$ROOT/deploy/deploy-native-staging.sh"
test -x "$SOURCE"
test -x "$IMPORT"
test -x "$RECOVERY"
grep -Fq 'recovery-ready' "$RECOVERY"
grep -Fq 'recovery-resume' "$RECOVERY"
grep -Fq 'adopt' "$RECOVERY"
grep -Fq 'hearth_quote_mysql_identifier "$schema"' "$RECOVERY"
grep -Fq 'HEARTH_STAGING_RECOVERY_ADOPT_SCHEMA' "$DEPLOY"
...
grep -Fq 'preserving it and refusing to overwrite uncertain migration state' "$SOURCE"
grep -Fq 'staging-dump-state.sh' "$SOURCE"
grep -Fq 'flock -x' "$SOURCE"
grep -Fq '.partial' "$SOURCE"
! grep -Fq 'sudo -n rm -f -- $dump_file' "$SOURCE"
